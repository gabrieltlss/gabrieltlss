## Gabriel Teles - 22 anos - Brasil 🌏

Estudante de programação focado em desenvolvimento web.

### Conhecimentos:
* HTML
* CSS / W3CSS
* JavaScript / TypeScript
* Node.JS
* MySQL
* React
* NextJS (em andamento)

### Projetos pessoais

|Projeto                                                                          |Descrição                                                                             |
|---------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
|👑 [App de Notas - Interface](https://note-app.gabrieltlss.com.br/)              |Interface do aplicativo de notas feito em React (Meu melhor projeto 🎖️)              |
|👑 [App de Notas - Camada Lógica](https://note-app.gabrieltlss.com.br/)          |API para aplicativo de notas feito em TypeScript (Meu melhor projeto 🎖️)             |
|🎹 [Classical Course](https://classical-app.gabrieltlss.com.br/)                 |Landing Page responsiva para exposição de curso de música (HTML, CSS e JS)            |
|📦 [Promos da Flávia](https://promos.flaviatlss.com.br/)                         |Landing Page dinâmica feito em JS, Node.JS, MySQL e EJS (web tradicional - monolítica)|
|✉️ [API para contato](https://github.com/gabrieltlss/gabrieltlss-contact-api.git)|Projeto simples para envio de e-mails usando o SendGrid (usado em meu site pessoal)   |

**Alguns dos endereços acima levam para a página web de cada projeto, que foram hospededos em uma VPS na Hostinger.**
**Para visualizar o repositório de cada projeto, visite meus repositórios públicos ou a aba de projetos fixados.**

### Contato
Aberto para propostas de trabalho.

Contato através de meu [site pessoal](https://gabrieltlss.com.br/).
